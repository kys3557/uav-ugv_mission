...
private:
  ...
  void handleRendezvous()
  {
    ...
    case RendezvousStep::LAND:
    {
      ...
      if (is_drone_stable_and_stopped_)
      {
        RCLCPP_INFO(this->get_logger(), "RendezvousStep: LAND - Vehicle has stopped. Mission phase is now Completed.");
        // send_land_command();
      ...
      if (!ids.empty())
      {
        ...
        for (size_t i = 0; i < ids.size(); ++i)
        {
          ...
          try
          {
            ...
            tf2::Vector3 v_camera_offset_in_base(0.05, 0.0, 0.1);
            ...
            // geometry_msgs::msg::PointStamped marker_point_camera;
            ...
            // marker_point_camera.point.x = -tvecs[i][2];
            // marker_point_camera.point.y = tvecs[i][0];
            // marker_point_camera.point.z = -tvecs[i][1];

            // geometry_msgs::msg::PointStamped marker_pose_map = tf_buffer_->transform(
            //     marker_point_camera,
            //     "map",
